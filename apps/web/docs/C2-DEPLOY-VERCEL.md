...
4. **Si el build falla con** “No Output Directory named `dist` found”: casi siempre es **Root Directory mal puesto**. En Vercel → **Settings** → **General** → **Root Directory** debe ser **`apps/web`** (no la raíz del repo). Si el proyecto quedó como Vite, en **Build & Development** pon **Framework Preset** → **Next.js** y borra **Output Directory** (o déjalo; en el repo `apps/web/vercel.json` fuerza `outputDirectory: ".next"` y en la raíz `vercel.json` fuerza Next + `apps/web/.next` por si el Root sigue siendo `.`).
